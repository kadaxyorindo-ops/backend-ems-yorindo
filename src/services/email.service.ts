...
const YORINDO_LOGO_CID = "yorindo-logo";
...
function getYorindoLogoAttachment(): EmailAttachment {
  return {
    filename: "yorindo-logo.png",
    content: readFileSync(LOGO_PATH),
    contentType: "image/png",
    cid: YORINDO_LOGO_CID,
  };
}
...
export async function sendCampaignEmail(params: {
  ...
  const uniqueRecipients = Array.from(
    new Map(
      params.recipients.map((recipient) => [recipient.email, recipient]),
    ).values(),
  );
