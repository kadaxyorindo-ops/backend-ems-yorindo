...
function toAnswerRowFromStructured(
  ...
  const field =
    ...
    (typeof item.key === "string" ? resolveFieldMeta(item.key, lookup) : null) ??
    (typeof item.label === "string" ? resolveFieldMeta(item.label, lookup) : null);
...
function isMeaningfulAnswerValue(value: unknown): boolean {
  ...
  if (Array.isArray(value)) return value.some((item) => isMeaningfulAnswerValue(item));
  return true;
}
...
function mapStructuredRegistrationAnswers(
  ...
  const fieldLookup = new Map(snapshotFields.map((field) => [field.fieldId, field]));
...
function normalizeRegistrationAnswers(
  ...
  const structuredAnswers = mapStructuredRegistrationAnswers(answers, snapshotFields);
  ...
  const fallbackLookup = new Map(
    eventFields
      ...
      .map((field) => [field.fieldId, field] satisfies [string, IRegistrationField]),
  );

  return normalizeSurveyResponseAnswers(answers, eventFields).filter((answer) => {
    const field = fallbackLookup.get(String(answer.questionId).trim());
    return Boolean(field && isMeaningfulAnswerValue(answer.value));
  });
}

export async function getEventSurveyAnswerSources(
  eventId: string,
): Promise<{
  ...
  const eventFields = (event.registrationForm?.fields ?? []) as IRegistrationField[];
    ...
    registrations.map((registration) => [
      String(registration.participantId),
      {
        answers: normalizeRegistrationAnswers(
          ...
          (registration.formSnapshot?.fields ?? []) as RegistrationSnapshotField[],
          eventFields,
        ),
      },
    ]),
  ...
  const registrationsWithStoredAnswers = registrations.filter(
    (registration) => Array.isArray(registration.answers) && registration.answers.length > 0,
  ).length;
  const surveyResponsesWithAnswers = surveyResponses.filter((response) => {
    ...
    return isObjectLike(response.answers) && Object.keys(response.answers).length > 0;
  }).length;

  surveyResponses.forEach((response) => {
    const participantId = String(response.participantId);
    const answers = normalizeSurveyResponseAnswers(response.answers, eventFields).filter(
      (answer) => isMeaningfulAnswerValue(answer.value),
    );
